...
public class UserController {
    ...
    @SystemControllerLog("进入主页")
    public ModelAndView getIndex(HttpSession session){
    	
        ModelAndView view = new ModelAndView("index");
        String userid =(String)session.getAttribute("userid");
        String nickname = userService.selNickname(userid);
		int count = recordsService.selectCountByUserid(userid);
		List<Records> list = recordsService.selectByUserid(userid); 			
     ...
     * @return
     */
    @SystemControllerLog("进入个人信息编辑页面")
    ...
    public String  update(@PathVariable("userid") String userid, @ModelAttribute("userid") String sessionid, UserInfo user, RedirectAttributes attributes,
                          NetUtil netUtil, LogUtil logUtil, CommonDate date, WordDefined defined, HttpServletRequest request){
        boolean flag = userService.update(user);
        if(flag){
//            logService.insert(logUtil.setLog(userid, date.getTime24(), defined.LOG_DETAIL_UPDATE_PROFILE, netUtil.getIpAddress(request)));
